...
class RDF_Parser extends RDF_Object
{
    ...
    function _generate_anonymous_uri(&$buf)
    {
        ...
        $buf = RDF_BNODE_PREFIX . $this->rdf_parser['anonymous_id'];
    }
        ...
        $object_type, $object, $xml_lang, $bag_id, $statements, $statement_id, $datatype)
    {
        ...
        if (!$xml_lang && $object_type == RDF_OBJECT_TYPE_LITERAL
            && isset($this->rdf_parser['document_xml_lang'])
        ) {
        ...
        $xml_lang, $bag_id, $statements)
    {
        ...
        for ($i = 0; isset($attributes[ $i ]); $i += 2) {
            ...
            } else if (RDF_XML_NAMESPACE_URI == $attribute_namespace_uri) {
    ...
    function _handle_resource_element($namespace_uri, $local_name, $attributes, $parent)
    {
        ...
        if (($local_name != RDF_DESCRIPTION) || ($namespace_uri != RDF_NAMESPACE_URI)) {
            $type = $namespace_uri;
            $type .= $local_name;
    ...
    function _handle_property_element(&$namespace_uri, &$local_name, &$attributes)
    {
        ...
        if (($namespace_uri == RDF_NAMESPACE_URI) && ($local_name == RDF_LI)) {
        ...
        if ($node_id) {
            ...
            $this->rdf_parser['top']['state'] = RDF_IN_PROPERTY_EMPTY_RESOURCE;
        }
        ...
        if ($parse_type) {
            ...
            if ($attribute_value == RDF_PARSE_TYPE_RESOURCE) {
                ...
                $this->rdf_parser['top']['state'] = RDF_IN_PROPERTY_PARSE_TYPE_RESOURCE;
                $this->rdf_parser['top']['subject_type'] = RDF_SUBJECT_TYPE_BNODE;
                $this->rdf_parser['top']['subject'] = $buffer;
                $this->rdf_parser['top']['bag_id'] = '';
                $this->rdf_parser['top']['datatype'] = $datatype;
            } elseif ($attribute_value == RDF_PARSE_TYPE_LITERAL) {
                $this->rdf_parser['top']['state'] = RDF_IN_PROPERTY_PARSE_TYPE_LITERAL;
                ...
                $this->rdf_parser['top']['state'] = RDF_IN_PROPERTY_PARSE_TYPE_COLLECTION;
    ...
    function _start_element_handler($parser, $name, $attributes)
    {
        ...
        case RDF_IN_TOP_LEVEL:
            // set base_uri, if possible
            foreach ($attributes as $key => $value)
            if ($key == RDF_XML_NAMESPACE_URI . RDF_NAMESPACE_SEPARATOR_CHAR . 'base') {
            ...
            if (RDF_NAMESPACE_URI . RDF_NAMESPACE_SEPARATOR_STRING . RDF == $name) {
                $this->rdf_parser['top']['state'] = RDF_IN_RDF;
        ...
        case RDF_IN_PROPERTY_PARSE_TYPE_COLLECTION:
            ...
            break;
        case RDF_IN_PROPERTY_UNKNOWN_OBJECT:
       ...
       an element, the object must be a resource, */
        ...
        case RDF_IN_PROPERTY_LITERAL:
            ...
            break;
        case RDF_IN_PROPERTY_PARSE_TYPE_LITERAL:
            $this->rdf_parser['top']['state'] = RDF_IN_XML;
            /* fall through */
        case RDF_IN_XML:
            $this->rdf_parser['xml_literal']['depth']++;
            $this->_handle_xml_start_element($namespace_uri, $local_name, $attributes);
            break;
        case RDF_IN_PROPERTY_RESOURCE:
            $this->_report_warning('only one element allowed inside a property element');
            break;
        case RDF_IN_PROPERTY_EMPTY_RESOURCE:
    ...
    {
        $parser = xml_parser_create_ns($encoding, RDF_NAMESPACE_SEPARATOR_CHAR);
        xml_parser_set_option($parser, XML_OPTION_CASE_FOLDING, 0);
        $this->rdf_parser['xml_parser'] = $parser;
        $this->rdf_parser['namespaces'][RDF_XML_NAMESPACE_URI] = 'xml';

        xml_set_object($this->rdf_parser['xml_parser'], $this);
        xml_set_element_handler(
            $this->rdf_parser['xml_parser'],
