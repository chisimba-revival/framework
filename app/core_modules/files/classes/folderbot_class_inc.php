    ...
    function scan($path) {
        ...
        while ($path = array_pop($stack)) { // as long as there are paths on stack
            ...
            while (($this->curfile = readdir($handle)) != false) { // as long as there are files in the folder
                if (!strcmp(".", $this->curfile)
                    )continue; // this may not the most efficient way to detect the . and .. entries
 if (!strcmp("..", $this->curfile)
                    )continue; // but it is the easiest to understand
 if (!strcmp("CVS", $this->curfile)
                    )continue;# ignore CVS folders
                if (!strcmp("config", $this->curfile)
                    )continue;# do not display config folder
                ...
                if (is_dir($this->curfile)) { // if a folder
                    if ($this->recurse
                        )array_push($stack, $this->curfile); // we push it onto the stack if we are recursing
 $this->folder_event(); // and we trigger an event
                } else {
                    if ($this->is_match()
                        )$this->file_event(); // if match, we trigger the file event
