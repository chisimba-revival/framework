...
if ((is_countable($files) ? count($files) : 0) == 0) {
    ...
    $converterPath=$objAltConfig->getModulePath()."/documentconverter/resources/jodconverter-2.2.0/lib/jodconverter-cli-2.2.0.jar";
    $checkOpenerScript = '
<script type="text/javascript">
function selectFile(file, id)
{
    if (window.opener) {
       ...
       // window.opener.document.getElementById("input_selectfile_'.$inputname.'").value = fileName[id];
        ...
        var presentationURL="'.$folderPath.'/"+fileName[id];
        var presentationPath="'.$converterPath.'";
        var presentationId=fileId[id];
        
        window.close();
        window.opener.passPresentationUrlToApplet(presentationURL,presentationPath,presentationId);
        window.opener.focus();
    } else {
