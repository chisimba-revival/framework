...
class multifileselect2 extends filemanagerobject
{
    /**
    ...
    * @return string
    */
    public function showJavaScript($name)
    {
...
function removeMultiFile'.$name.'(name)
{
    ...
    if (confirm("Remove this file?")) {
        // Get list of items
        defaultVal = jQuery(\'#hidden_headerscripts_'.$name.'\').attr(\'value\');
...
function addToMultiList'.$name.'()
{
    ...
    defaultText = jQuery(\'#multifileslist_'.$name.'\').html();
    ...
    newVal = jQuery(\'#hidden_temp_multiselect_'.$name.'\').attr(\'value\');
    ...
    if (newVal != null) {
        ...
        // 
        jQuery(\'#multifileslist_'.$name.'\').html(defaultText+str);
        
        clearFileInputJS(\'temp_multiselect_'.$name.'\');
    }
}
</script>';
    ...
    */
    ...
    {
        $this->appendArrayVar('headerParams', $this->showJavaScript($this->name));
            ...
            foreach ($scripts as $script)
            {
                ...
                if (trim($script) != '') {
                    ...
                    if ($fileInfo != FALSE) {
                        ...
                        $defaultDisplay .= '<span class="multiitem_'.$fileInfo['id'].'">'.$fileInfo['filename'].' <a href="javascript:removeMultiFile'.$this->name.'(\''.$fileInfo['id'].'\');">'.$this->deleteIcon.'</a><br /></span>';
                    }
                }
            }
        }
        ...
        $input->cssId = 'hidden_headerscripts_'.$this->name;
        $input->extra = ' size="60"';
        $input->value = $defaultValue;
        $input->fldType = 'hidden';
        
        $selectFile = $this->newObject('selectfile', 'filemanager');
        $selectFile->name = 'temp_multiselect_'.$this->name;
        ...
        $addButton->setOnClick('addToMultiList'.$this->name.'('.$this->name.');');
        
        $str = '<div id="multifiles_'.$this->name.'">';
        $str .= '<div id="multifileslist_'.$this->name.'">'.$defaultDisplay.'</div>';
