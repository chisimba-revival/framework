...
class upload extends filemanagerobject {
    ...
    public function uploadFile($fileInputName, $ext = NULL, &$fileUploadResultsArray = NULL) {
        ...
        // Also check that same file is not upload twice in a multiple upload environment
            ...
            //check the permissions on the subfolder..specifically if private switch
            //to private location

            $uploadFolderDetails = $this->objFolder->getFolderByPath($this->uploadFolder);
            
            $isSecure = false;
            if ($this->objFolderAccess->isFileAccessPrivate($uploadFolderDetails)) {
                $isSecure = true;
                // Determine whether to include subfolders
                if ($this->useFileSubFolder) {
                    // Create Full Server Path to Uploaded File
                    $savepath = $this->secureFolder . '/' . $this->uploadFolder . $subfolder . '/';
                } else {
                    // Create Full Server Path to Uploaded File
                    $savepath = $this->secureFolder . '/' . $this->uploadFolder . '/';
                }
            }
...
////////////////////////////////////////////////////////////////////
            ...
            if (file_exists($savepath)) {
                ...
                if (is_array($originalFile)) {
                    ...
                    if ($isSecure) {
                        $savePath = $this->secureFolder . '/filemanager_tempfiles/' . $originalFile['id'];
                        $this->objMkdir->mkdirs($savepath, 0777);
                    } else {
                        $savePath = $this->objConfig->getcontentBasePath() . '/filemanager_tempfiles/' . $originalFile['id'];
                    }
            ...
            if ($addToDatabaseAndIndex) {
                ...
                // 1) Add to Database
                $fileId = $this->objFile->addFile(
                        $filename, $path, $file['size'], $file['type'], $subfolder, $version, $this->objUser->userId(), $this->getParam('cite_' . $fileInputName, ''), $this->getParam('creativecommons_' . $fileInputName, ''));

                //update access type
                if ($isSecure) {
                    $this->objFile->setFileAccess($fileId, 'private_all');
                }
