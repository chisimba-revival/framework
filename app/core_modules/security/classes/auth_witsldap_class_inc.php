...
// Build up an array of the user's info
            ...
            if (isset($ldapData['mail'][0])) {
                ...
                /**
                 * Process wits integration
                 */
                $this->autoenroll=true;
                $this->processLDAPIntegration($info);
            }
    ...
    function processLDAPIntegration() {
        /**
        ...
        $isRegistered = $objModule->checkIfRegistered('userextra');
        $groupidarr=$this->getArray("select * from tbl_perms_groups where group_define_name='elearnadmins'");
        ...
        $receivers =$this->getArray($sql);

        if($isRegistered) {
            $userextra=$this->getObject('userinfo','userextra');
            $userextra->process($info,$groupidarr,$receivers);
        }

    }
    function sendMailNotification($names) {
    ...
    public function getUserLdapDetails($username) {
        ...
        $ldapconn = ldap_connect ($this->ldapserver[$this->currentIndex], $this->ldapPort[$this->currentIndex]) or die ('LDAP CONNECTION FAILED');
...
//loop through till you get server to connect ti
        foreach($this->ldapserver as $server) {
            ...
            $ldapconn = ldap_connect($server, $this->ldapPort[$index]) or die ('LDAP CONNECTION FAILED');
