...
class dbfiledata extends dbTable {
     ...
     */
    public function insertFileData($fileId, $parentId, $path) {
        $completepath = $path;
        ...
        while ( ! feof ( $fp ) ) {
            ...
            $this->insert ( array ('tbl_context_file_tbl_context_parentnodes_id' => $parentId, 'tbl_context_file_id' => $fileId, 'segment' => $count, 'filedata' => $binarydata ) );
            $count = $count + 1;
        }
        fclose ( $fp );
    }
     ...
     * @param string $path     The the path the the file
     */
    public function updateFileData($fileId, $parentId, $path) {
