...
class dbfile extends dbTable {
     ...
     * @return array
     */
    public function getFiles($contextCode = NULL, $category = NULL, $orderBy = 'filedate DESC') {
...
" OR category='documents'
     ...
     * @return boolean
     */
    public function insertFile($parentId, $params, $filepath) {
        $title = $params ['title'];
        $name = $params ['name'];
        ...
        $fileId = $this->insert ( array ('tbl_context_parentnodes_id' => $parentId, 'title' => $title, 'name' => $name, 'filedate' => $filedate, 'description' => $description, 'version' => $version, 'size' => $size, 'datatype' => $datatype, 'path' => $path, 'category' => $category ) );
        if ($path == '') {
     ...
     * @return boolean
     */
    public function updateFile($id, $params) {
